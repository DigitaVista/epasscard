...
class EPC_Loyalty_Rule_Service {
	...
	public static function get_rules() {
		...
		if ( empty( $rules ) ) {
			$legacy = EPC_Loyalty_Order_Service::get_settings();
			$rules  = array(
				array(
					'id'                  => 'default',
					'name'                => __( 'Default earning rule', 'epasscard' ),
					'active'              => true,
					'priority'            => 10,
					'stack'               => true,
					'award_type'          => 'per_currency',
					'points'              => $legacy['points_per_currency'],
					'minimum_spend'       => 0,
					'point_expiry_days'   => 0,
				),
			);
	...
	private static function matches_order( array $rule, $order ) {
		...
		if ( ! empty( $rule['first_order'] ) && function_exists( 'wc_get_customer_order_count' ) && wc_get_customer_order_count( $user_id ) > 1 ) {
			return false;
		}
