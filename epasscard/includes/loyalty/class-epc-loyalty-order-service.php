...
class EPC_Loyalty_Order_Service {
	...
	/**
	 * Credit one existing shop order: award if eligible, then apply refunds.
	 *
	 * Used by historical sync. Live hooks still call award_order() directly.
	 *
	 * @param \WC_Order $order Order object.
	 * @return array{awarded: int, skipped: int, refunded: int}|\WP_Error
	 */
	public static function credit_existing_order( $order ) {
		if ( ! $order instanceof WC_Order ) {
			return array(
				'awarded'  => 0,
				'skipped'  => 1,
		...
		$awardable = self::is_qualifying_status( $status ) || 'refunded' === $status;
		if ( ! $awardable || (int) $order->get_user_id() <= 0 ) {
			return array(
				'awarded'  => 0,
				'skipped'  => 1,
				'refunded' => 0,
		...
		$before  = EPC_Loyalty_Ledger_Service::get_by_event_key( self::award_event_key( $order->get_id() ) );
		$award   = self::award_order( $order, array( 'require_qualifying_status' => self::is_qualifying_status( $status ) ) );
		...
		foreach ( $order->get_refunds() as $refund ) {
			...
			self::on_order_refunded( $order->get_id(), $refund->get_id() );
			++$refunded;
		}

		$after = EPC_Loyalty_Ledger_Service::get_by_event_key( self::award_event_key( $order->get_id() ) );
		return array(
			'awarded'  => ( ! $before && $after ) ? 1 : 0,
			'skipped'  => ( $before || ! $after ) ? 1 : 0,
			'refunded' => $refunded,
		);
	}
	 ...
	 */
	public static function award_order( $order, array $args = array() ) {
		$require_status = ! array_key_exists( 'require_qualifying_status', $args ) || (bool) $args['require_qualifying_status'];
		if ( ! $order instanceof WC_Order || ( $require_status && ! self::is_qualifying_status( $order->get_status() ) ) ) {
			return null;
		}
