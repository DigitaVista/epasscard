...
class EPC_Loyalty_Customers_List_Table extends WP_List_Table {
	...
	/**
	 * Output a table row with a stable user id for AJAX updates.
	 *
	 * @param array<string, mixed> $item Customer summary.
	 * @return void
	 */
	public function single_row( $item ) {
		echo '<tr data-epc-user-id="' . esc_attr( (string) ( $item['user_id'] ?? '' ) ) . '">';
	...
	private function render_actions( array $item ) {
		...
		$html .= ' <button type="button" class="button button-small epc-loyalty-history-open" data-user-id="' . esc_attr( (string) $user_id ) . '" data-name="' . esc_attr( (string) $item['display_name'] ) . '">' . esc_html__( 'History', 'epasscard' ) . '</button>';
