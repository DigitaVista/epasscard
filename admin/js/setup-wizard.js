(function ($) {
	...
	function setInstalling(on) {
		installing = !!on;
		busy(on);
		$root().toggleClass('is-installing', installing);
		$root().find('[data-epc-wizard-back], [data-epc-wizard-skip], [data-epc-wizard-continue-connected], .epc-wizard__exit').prop('disabled', installing);
		if (installing) {
			$root().find('.epc-wizard__exit').attr('aria-disabled', 'true');
		} else {
			$root().find('.epc-wizard__exit').removeAttr('aria-disabled');
		}
	}

	function guardBusy(event) {
		if (!installing) {
			return false;
		}
		event.preventDefault();
		event.stopImmediatePropagation();
		status(epcWizard.i18n.installing || 'Installing…', 'success');
		return true;
	}
	...
	function showPanel(id) {
		...
		$wrap.find('[data-panel]').each(function () {
			if (this.getAttribute('data-panel') === id) {
	...
	function restartCelebrate() {
		var $stage = $root().find('.epc-wizard__celebrate');
		if (!$stage.length) {
			return;
		}
		$stage.removeClass('is-playing');
		// Force reflow so the fireworks animation can restart after AJAX swaps.
		void $stage.get(0).offsetWidth;
	...
	function initPanel() {
		...
		if (($root().attr('data-epc-step') || epcWizard.step) === 'done') {
			restartCelebrate();
		}
	}
	...
	$(function () {
		...
		$(document).on('click', '[data-epc-wizard-complete]', function () {
			...
			post('epc_wizard_complete')
				.done(function (resp) {
					handleStepResponse(resp, epcWizard.i18n.complete || '');
				})
				.fail(function (xhr) {
					status(failMessage(xhr), 'error');
				})
		...
		$(document).on('click', '[data-epc-wizard-back]', function (event) {
			if (guardBusy(event)) {
				return;
			}
			busy(true);
		...
		$(document).on('click', '[data-epc-wizard-continue-connected]', function (event) {
			if (guardBusy(event)) {
				return;
			}
			busy(true);
		...
		$(document).on('click', '[data-epc-wizard-skip]', function (event) {
			if (guardBusy(event)) {
				return;
			}
			busy(true);
		...
		$(document).on('click', '.epc-wizard__exit', function (event) {
			if (guardBusy(event)) {
				return;
			}
		});
		...
		$(document).on('mousedown', '[data-epc-install], a[data-epc-stop-label]', function (event) {
			event.stopPropagation();
		});

		$(document).on('click', 'a[data-epc-stop-label]', function (event) {
			event.stopPropagation();
		});

		$(document).on('click', '[data-epc-install]', function (event) {
			event.preventDefault();
			event.stopPropagation();
			...
			var $btn = $(this);
			var key = $btn.attr('data-epc-install') || '';
			var installStatus = $btn.attr('data-epc-install-status') || 'missing';
			var $pill = $btn.closest('.epc-wizard__plugin');
			var activateLabel = $btn.text();

			if (!key) {
				return;
			}

			if (!epcWizard.canInstall) {
				status(epcWizard.i18n.installDenied, 'error');
				return;
			}
			...
			$btn.prop('disabled', true).text(
				installStatus === 'installed'
					? epcWizard.i18n.activating
					: epcWizard.i18n.installing
			);
			status(
				installStatus === 'installed'
					? epcWizard.i18n.activating
			...
			post('epc_wizard_install_plugin', { key: key })
				.done(function (resp) {
					if (!resp || !resp.success) {
						status((resp && resp.data && resp.data.message) || epcWizard.i18n.error, 'error');
						$pill.removeClass('is-busy');
						$btn.prop('disabled', false).text(activateLabel);
						setInstalling(false);
						return;
					}

					status((resp.data && resp.data.message) || epcWizard.i18n.installed, 'success');

					if (resp.data && resp.data.needs_reload) {
						window.location.href = (resp.data.reloadUrl) || window.location.href;
						return;
					}
